...
const string kDataFilename("data/case-bc64e0e8/case.dat");

namespace common {

template<>
class SerDe<Case_bc64e0e8>
{
public:
    void encode(const Case_bc64e0e8& in) const
    {
    ...
    void encode(vector<uint8_t>* out, const Case_bc64e0e8& in) const
    {
        auto_ptr<avro::OutputStream> os = avro::memoryOutputStream();
        avro::EncoderPtr enc = avro::binaryEncoder();
        enc->init(*os);
...
int main(int argc, char** argv) {
    ...
    execute<Case_bc64e0e8>(
        gen,
        common::SerDe<Case_bc64e0e8>(),
        opt.encRepeat,
        opt.decRepeat);
    return 0;
}
